...
/**
 * Fetch vessel positions from AIS Stream
 * Uses WebSocket connection to aisstream.io for real-time data
 * Requires VITE_AISSTREAM_API_KEY environment variable
 * Get a free API key at https://aisstream.io
 */
export async function fetchVesselPositions(): Promise<Vessel[]> {
	// Import vessel stream service dynamically to avoid SSR issues
	const { getVesselPositions, connectVesselStream, vesselConnectionStatus } = await import(
		'$lib/services/vessel-stream'
	);
	const { get } = await import('svelte/store');

	// Ensure WebSocket is connected
	const status = get(vesselConnectionStatus);
	if (status === 'disconnected') {
		connectVesselStream();
	}

	// Return current vessel positions from the stream
	return getVesselPositions();
}
