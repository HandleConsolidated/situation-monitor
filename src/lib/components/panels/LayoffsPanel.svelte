...
<Panel id="layoffs" title="Layoffs Tracker" {count} {loading} {error}>
	{#if layoffs.length === 0 && !loading && !error}
		<div class="text-center text-slate-400 text-[10px] sm:text-xs py-4">No recent layoffs data</div>
	{:else}
		<div class="flex flex-col">
			{#each layoffs as layoff, i (layoff.company + i)}
				...
				>
					<div class="text-xs sm:text-sm font-bold text-white mb-0.5">{layoff.company}</div>
					{#if layoff.count}
						<div class="text-[10px] sm:text-xs font-mono text-red-500 tabular-nums mb-1">
					...
					<div class="flex justify-between items-start gap-2">
						<span class="text-[10px] sm:text-xs text-slate-400 leading-tight flex-1">{layoff.title}</span>
						<span class="text-[10px] sm:text-xs font-mono text-slate-500 whitespace-nowrap"
